    ...
    if choice=="📁FileUploader":
        ...
        if uploaded_file is None:
            ...
            if df is not None:
                ...
                st.download_button( label="Download smart clean dataset",data=csv_data,file_name="cleaned_data.csv",mime="text/csv")
            elif txt_data:
               text_bytes = txt_data.encode('utf-8')
               st.download_button( label="Download smart clean dataset",data=text_bytes,file_name="cleaned_data.csv",mime="text/csv")
        ...
        if uploaded_file is None:
            ...
            if df is not None:
                # Show cleaned preview
                csv_data = df.to_csv(index=False).encode('utf-8')
                st.download_button( label="Download full clean dataset",data=csv_data,file_name="cleaned_data.csv",mime="text/csv")
            elif txt_data:
               text_bytes = txt_data.encode('utf-8')
               st.download_button( label="Download full clean dataset",data=text_bytes,file_name="cleaned_data.csv",mime="text/csv")
    if choice== "❓About":
